...
class RAGVectorSearch:
    ...
    def __init__(self, config: RAGSearchConfig = None):
    ...
    def get_tag_categories(self) -> List[str]:
        """獲取可用的標籤類別"""
        if not self.text_processor.tag_processor:
            return []
...
# 工廠函數
def create_rag_vector_search(config: RAGSearchConfig = None) -> RAGVectorSearch:
    """創建 RAG 向量搜尋服務"""
