        ...
        try:
            ...
            with self.conn.cursor() as cursor:
                ...
                return {
                    ...
                    'action': 'inserted' if result_id == mapped_data['podcast_id'] else 'updated',
                    'data': mapped_data
                }
                        ...
                        for idx, episode in enumerate(batch):
                            try:
                                ...
                                cursor.execute("""
                                    ...
                                    ON CONFLICT (podcast_id, episode_title) DO NOTHING
                                """, (
                                    mapped_data['podcast_id'],
...
def main():
    """主程式"""
    logger.info("=== 穩健批次上傳 batch_input 資料夾到 PostgreSQL ===")
    ...
    try:
        # 建立上傳器
        uploader = RobustUploader(config)
        ...
        # 儲存報告到檔案
        report_file = os.path.join(os.path.dirname(input_folder), 'robust_upload_report.txt')
