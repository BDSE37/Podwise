#!/usr/bin/env python3
"""
修復版本的 PostgreSQL 欄位 mapping 和重複檢查腳本
解決 SQL 語法錯誤和交易問題
"""
...
class FixedMappingChecker:
    """修復版本的欄位 mapping 和重複檢查類別"""
        ...
        try:
            ...
            if podcast_id:
                try:
                    ...
                    logger.warning(f"檢查 podcast_id 重複失敗: {e}")
    ...
    def analyze_rss_mapping(self, json_file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # 檢查重複的 episodes（只檢查前100個以避免過多查詢）
            episode_titles = [ep.get('title', '') for ep in episodes_data[:100] if ep.get('title')]
            ...
            if episode_titles:
                try:
                    ...
                    logger.warning(f"檢查 episode 重複失敗: {e}")
            ...
            return {
                ...
                'duplicate_analysis': {
                    ...
                    'existing_titles': len(duplicate_titles),
    ...
    def generate_report(self, results: Dict[str, Any]) -> str:
        ...
        report = []
        report.append("=" * 60)
        report.append("PostgreSQL 欄位 Mapping 和重複檢查報告（修復版本）")
        report.append("=" * 60)
...
def main():
    """主程式"""
    logger.info("=== PostgreSQL 欄位 Mapping 和重複檢查（修復版本）===")
    ...
    try:
        # 建立檢查器
        checker = FixedMappingChecker(config)
        ...
        # 儲存報告到檔案
        report_file = os.path.join(os.path.dirname(input_folder), 'fixed_mapping_check_report.txt')
