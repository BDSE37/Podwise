...
class ImprovedSafeUploader:
    ...
    def _map_episode_fields(self, episode_data: Dict[str, Any], podcast_id: int) -> Dict[str, Any]:
        ...
        # 處理日期
        published_date = None
        date_fields = ['published_date', 'published', 'pub_date', 'date']
...
def main():
    """主程式"""
    logger.info("=== 改進的安全批次上傳 batch_input 資料夾到 PostgreSQL ===")
    ...
    try:
        # 建立上傳器
        uploader = ImprovedSafeUploader(config)
        ...
        # 儲存報告到檔案
        report_file = os.path.join(os.path.dirname(input_folder), 'improved_safe_upload_report.txt')
