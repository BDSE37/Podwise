...
2. 清理表情符號和特殊字元
3. 切分依據空白 \n chunks
4. 基於 TAG_info.csv 進行標籤匹配（最少一個，最多三個）
5. 如果沒有匹配標籤，使用智能標籤系統
6. 生成向量嵌入
7. 遵循 Milvus 欄位規則儲存到 Milvus
8. 錯誤記錄和處理
"""
...
class CollectionProcessor:
    ...
    def create_milvus_collection(self, collection_name: str = "podcast_chunks") -> str:
        """
        ...
        try:
            ...
            fields = [
                ...
                # 向量標籤欄位
                FieldSchema(name="tag_1", dtype=DataType.FLOAT_VECTOR, dim=1024),
                FieldSchema(name="tag_2", dtype=DataType.FLOAT_VECTOR, dim=1024),
                FieldSchema(name="tag_3", dtype=DataType.FLOAT_VECTOR, dim=1024)
            ]
            
            # 建立 collection
            schema = CollectionSchema(
                fields=fields,
                description=f"Podcast Chunks Collection with Vector Tags (dim: 1024)"
            )
            ...
            logger.info(f"成功建立 Milvus collection: {collection_name}")
            return collection_name
            
        except Exception as e:
            logger.error(f"建立 Milvus collection 失敗: {e}")
            raise
        ...
        try:
            ...
            for item in data:
                insert_data.append({
                    ...
                    'tag_1': item['tag_1'].tolist(),
                    'tag_2': item['tag_2'].tolist(),
                    'tag_3': item['tag_3'].tolist()
                })
    ...
    def generate_tag_vectors(self, tags: List[str], default_embedding: np.ndarray) -> List[np.ndarray]:
        """
        生成標籤向量
        
        Args:
            tags: 標籤列表
            default_embedding: 預設嵌入向量
        ...
        for tag in tags:
            try:
                # 為標籤生成向量
                tag_embedding = self.embedding_model_instance.encode([tag], normalize_embeddings=True)[0]
                tag_vectors.append(tag_embedding)
            except Exception as e:
                logger.warning(f"生成標籤向量失敗，使用預設向量: {e}")
                ...
                try:
                    ...
                    for i, chunk in enumerate(chunks):
                        try:
                            ...
                            # 生成主要嵌入向量
                            main_embedding = self.generate_embeddings([chunk])[0]
                            
                            # 生成標籤向量
                            tag_vectors = self.generate_tag_vectors(tags, main_embedding)
