...
from process_collections_simple import CollectionProcessor, TextCleaner, TagProcessor
...
def test_tag_processor():
    ...
    processor = TagProcessor("TAG_info.csv")
...
def test_text_chunking():
    ...
    try:
        ...
        for i, chunk in enumerate(chunks):
            ...
            # 測試標籤提取
            tags = processor.tag_processor.extract_tags(chunk)
            print(f"提取的標籤: {tags}")
            print(f"標籤數量: {len(tags)}")
    ...
    try:
        ...
        if collection_name in db.list_collection_names():
            ...
            if doc_count > 0:
                ...
                # 獲取前 2 個文件
                test_docs = list(collection.find({}).limit(2))
                ...
                for i, doc in enumerate(test_docs):
                    ...
                    if text:
                        ...
                        if chunks:
                            ...
                            # 提取標籤
                            tags = processor.tag_processor.extract_tags(first_chunk)
                            print(f"提取的標籤: {tags}")
                            print(f"標籤數量: {len(tags)}")
                    else:
                        print("沒有文本內容")
