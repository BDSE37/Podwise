...
# Podwise RAG Pipeline Podman K8s 部署腳本
# 用於使用 Podman 構建 Docker 映像並部署到 K8s 集群
...
# 構建 Podman 映像
build_podman_image() {
    log_info "使用 Podman 構建 Docker 映像..."
    ...
    log_info "從專案根目錄構建映像..."
    cd "$PROJECT_ROOT"
    podman build -f deploy/k8s/rag-pipeline/Dockerfile -t $REGISTRY/$IMAGE_NAME:$IMAGE_TAG .
    BUILD_RESULT=$?
    cd "$SCRIPT_DIR"
    
    if [ $BUILD_RESULT -eq 0 ]; then
        log_success "Podman 映像構建成功"
    else
        log_error "Podman 映像構建失敗"
        exit 1
    fi
}
...
# 推送 Podman 映像
push_podman_image() {
    log_info "推送 Podman 映像到註冊表..."
    
    # 檢查註冊表連接
    if ! podman push $REGISTRY/$IMAGE_NAME:$IMAGE_TAG; then
        log_error "推送 Podman 映像失敗"
        exit 1
    fi
    
    log_success "Podman 映像推送成功"
}
...
show_deployment_info() {
    ...
    echo "podman ps -a | grep $IMAGE_NAME"
}

# 主函數
main() {
    log_info "開始 Podman K8s 部署流程..."
