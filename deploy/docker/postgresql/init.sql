-- Podwise PostgreSQL 完整初始化腳本
-- 帳號: bdse37
-- 密碼: 111111
-- 資料庫: podcast

-- 連接到 podcast 資料庫（已在環境變數中指定）
...
CREATE TABLE IF NOT EXISTS query_log_summary (
...
CREATE TABLE IF NOT EXISTS hot_episode_daily (
    date DATE NOT NULL,
    episode_id INTEGER NOT NULL,
...
CREATE TABLE IF NOT EXISTS alert_history_log (
...
CREATE INDEX IF NOT EXISTS idx_user_feedback_episode_id ON user_feedback(episode_id);
CREATE INDEX IF NOT EXISTS idx_cf_matrix_user_id ON cf_matrix(user_id);
CREATE INDEX IF NOT EXISTS idx_cf_matrix_episode_id ON cf_matrix(episode_id);
CREATE INDEX IF NOT EXISTS idx_hot_episode_daily_date ON hot_episode_daily(date);
CREATE INDEX IF NOT EXISTS idx_task_log_status ON task_log(status);
CREATE INDEX IF NOT EXISTS idx_embedding_log_episode_id ON embedding_log(episode_id);
...
GROUP BY p.podcast_id, p.name, p.author, p.category, p.created_at;

-- 授權
GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO bdse37;
GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public TO bdse37;
GRANT ALL PRIVILEGES ON ALL FUNCTIONS IN SCHEMA public TO bdse37;
